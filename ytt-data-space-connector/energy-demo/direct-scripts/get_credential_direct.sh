...
offer_uri=$(curl -s -k  -X GET "$1/realms/test-realm/protocol/oid4vc/create-credential-offer?credential_configuration_id=$2&pre_authorized=true" \
...
token_response=$(curl -s -k  -X POST "$1/realms/test-realm/protocol/openid-connect/token" \
  --header 'Accept: */*' \
  --header 'Content-Type: application/x-www-form-urlencoded' \
